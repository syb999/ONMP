...
url_WordPress="https://cn.wordpress.org/wordpress-6.3-zh_CN.zip"

# (3) Owncloud（经典的私有云）
url_Owncloud="https://download.owncloud.com/server/stable/owncloud-10.13.0.zip"
...
* ) echo "没有这个选项"; exit;;
    ...
    if [[ ! -d "/opt/wwwroot/$webdir" ]] ; then
        ...
        if [[ ! -f /opt/wwwroot/$name.$suffix ]]; then
            ...
            curl -k -o /opt/tmp/$name.$suffix $filelink
